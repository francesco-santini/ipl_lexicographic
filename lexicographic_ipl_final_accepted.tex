...
\begin{remark}\label{inverse}
	...
	with $+\infty \odiv +\infty = +\infty$ and  $z \odiv +\infty =  -\infty$ for all $z \in  \mathbb{Z} \cup \{-\infty\}$.
...
\label{usualNat}
	...
	$\bot$-divisors. Similarly, nothing can be stated for $a \odiv \top$.
	Consider e.g. the CLM $\mathcal{P}$ 
	of Example~\ref{setCLM}: we have $\emptyset \odiv X = V\setminus X$
	and $X \odiv V = X$; instead, in the CLM $\mathcal{Z}$ of Remark~\ref{inverse}
	we have $-\infty \odiv n = -\infty = n \odiv +\infty$.
