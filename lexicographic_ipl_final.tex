...
\end{proposition}
\begin{proof}
	The proof is by induction on $k$. Concerning the order, it is straightforward.
	...
	\bigcup_{i < n+1}I(A)^{i} A \{\bot_\mathcal{A}\}^{n-i}$
	and we are done, where the latter equality holds since
	$I(A)\{\bot_\mathcal{A}\}^{n+1} \subseteq \bigcup_{i < n}I(A)^{i+1} A \{\bot_\mathcal{A}\}^{n-i}$
	and $A = I(A) \cup C(A)$.
\end{proof}

As for the binary product, the top element can be 
either $\langle \top_\mathcal{A}, \bot_\mathcal{A}, \ldots, \bot_\mathcal{A}  \rangle$ or $\top^k_\mathcal{A}$,
...
$i$-th component $a_i$ and with $t_{\mid i}$ its prefix $\langle a_1, \ldots, a_i \rangle$, with the obvious 
generalisation of the latter for a set $X \subseteq A^k$.
...
that partial function $\delta$ being respectively defined or undefined on the inputs $s,t$,
...
\subsection{$\omega$-iterated product}\label{sec:infinite}
