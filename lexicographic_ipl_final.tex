...
%
Our focus was to show that the framework can include also lexicographic orders extended to tuples of any length, by systematising the approach initiated in \cite{sca}.
Such infinite tuples can be used, for example, with infinitely many timed-variables, therefore associated with an infinite stream of preferences. An application is represented by the definition of (constraint) automata accepting an infinite data stream as input~\cite{sca}.
...
We would like to thank the anonymous reviewers for pointing us to a simpler and clearer approach to proving lexicographic orders of infinite length. The first author \textcolor{red}{PROGETTI}, and the second author was supported by GNCS – ``Gruppo Nazionale per il Calcolo Scientifico'' - INdAM.
