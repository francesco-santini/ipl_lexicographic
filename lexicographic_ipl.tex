...
\end{abstract}
        \begin{keyword}
        	Residuation theory,  Lexicographic orders. %\sep Bucket Elimination.
        \end{keyword}
    \end{frontmatter}
...
The paper has the following structure. In Section~\ref{sec:om} we present  the background on partially ordered residuated monoids, the structure 
...
\begin{definition}
	...
	with  $k+1$ as result whenever the set is empty.
\end{definition}
...
In this paper we considered a formal framework for  SCSPs based on a residuated monoid of partially ordered preferences.  More specifically, the framework is dedicated to lexicographic orders.
Similar valued structures in the literature, such as pioneering ones as \cite{jacm97,schiex}, cannot straightforwardly deal with preferences managed as lexicographic orders, while successive proposals can~\cite{GadducciHMW13,valuation}. This work fills the gap by extending residuation~\cite{residuation1} also to such a class of preferences. This allows for using the classical solving algorithms that need some preference removal, as for instance arc consistency where values need to be moved 
...
In the future we plan to design specific heuristics to improve the solution of SCSPs based on such orders. 
